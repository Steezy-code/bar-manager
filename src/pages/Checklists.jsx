...
export default function Checklists() {
  ...
  return (
    <div className="space-y-6 pb-24 lg:pb-0">
      ...
      <div className="flex gap-2 overflow-x-auto flex-wrap">{Object.keys(tasks).map(t => <div key={t} className="flex items-center"><button onClick={() => setList(t)} className={`px-4 py-2 rounded-l-lg ${list===t?'bg-bar-accent':'bg-bar-card'}`}>{t}</button>{Object.keys(tasks).length > 1 && <button onClick={() => deleteList(t)} className="px-2 py-2 rounded-r-lg bg-red-600 text-white hover:bg-red-500">×</button>}</div>)}</div>
      <div className="card">
